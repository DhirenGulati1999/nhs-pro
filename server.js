...
const hostname = "sprint-procoreapi.newhomesourceprofessional.com";
...
app.prepare().then(() => {
  createServer((req, res) => {
    ...
    if (parsedUrl.pathname.toLowerCase().includes("/sso/startsso")) {
      ...
      const externalApiReq = http.request(
        externalApiOptions,
        (externalApiRes) => {
          let data = "";
      ...
      parsedUrl.pathname.toLowerCase().includes("/sso/assertionconsumerservice")
    ) {
      // Parse the form data
